...
export const Marker = (props: MarkerProps) => {
    ...
    return (
        <YMapMarker
            coordinates={getPolygonCenter(props.area.coordinates)}
            properties={{ hint: props.area.name }}
        >
            <div className="relative w-4 h-2">
                {props.getMapLocation().zoom <= ZOOM_THRESHOLD &&
                    <MarkerImage
                        area={props.area}
                        openObjectInfo={() => props.setSelectedArea(props.area)}
                    />}
